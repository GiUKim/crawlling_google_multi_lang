...
def crawlling_from_chrome(input_string, save_dir):
    ...
    #         urllib.request.urlretrieve(imgUrl, os.path.join(directory + save_dir, time.strftime("%d_%H%M%S") + '_' + str(count) + ".jpg"))
    #         count += 1
    #     except:
    #         pass

directory = 'C:\\Users\\AI\\PycharmProjects\\crawl_img\\datasets\\'

driver = webdriver.Chrome()

search_keyword_list = ['burning car photo', 'snow landscape']
...
for input in search_keyword_list:
    ...
    for lang in translate_language_list:
        if first_english:
            ...
            time.sleep(1)
            driver.find_element_by_xpath('//*[@id="tw-source-text-ta"]').send_keys(input)
            time.sleep(1)
            result = driver.find_element_by_xpath('//*[@id="tw-target-text"]/span').text
            print(input, ' -> ',  result)

        crawlling_from_chrome(result, 'snow')
